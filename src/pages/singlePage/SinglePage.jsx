...
function SinglePage() {
  ...
  // ─── 🔥 CHAT AND MESSAGE PROTECTION SYSTEM WITH ROUTER STATE ────────────────────────
  const handleMessageOwner = async () => {
    if (!user) {
      navigate("/auth/login");
      return;
    ...
    try {
      ...
      // Backend response se dynamic chat id nikalte hain
      const chatData = res.data.data || res.data;
      const targetChatId = chatData.id || chatData._id;

      // Profile Page ko state pass kar rahe hain taake chat auto open ho sake
      navigate("/profile", {
        state: {
          autoOpenChatId: targetChatId,
          receiverId: ownerId,
        },
      });
    } catch (err) {
      console.error("Chat backend redirection:", err);
      // Fallback: Agar pehle se chat bani ho toh direct profile par context bhej dein
      navigate("/profile", { state: { receiverId: ownerId } });
    } finally {
      setChatLoading(false);
    }
  };
