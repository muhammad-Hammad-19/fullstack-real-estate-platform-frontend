...
//       socket.off("getMessage", handleLiveIncoming);
//     };
//   }, [activeChat, increaseNotification]);
...
//           axios.get(`${BASE_URL}/api/posts/user/${userId}`, {
//             withCredentials: true,
//           }),
//           axios.get(`${BASE_URL}/api/users/savedPosts`, {
//             withCredentials: true,
//           }),
...
//         );
//         await axios.put(
//           `${BASE_URL}/api/chats/read/${chatId}`,
//           {},
//           { withCredentials: true },
//         );
...
//         `${BASE_URL}/api/messages/${currentActiveChatId}`,
...
//                     alt="User Avatar"
//                     className="w-full h-full rounded-full object-cover border-4 border-white shadow-md block"
//                   />
//                 ) : (
//                   <div className="w-full h-full rounded-full bg-slate-900 text-white border-4 border-white shadow-md flex items-center justify-center font-bold text-2xl select-none">
...
//                         {hasAvatar(chat.receiver?.avatar) ? (
//                           <img
//                             src={resolveAvatar(chat.receiver?.avatar)}
//                             alt=""
//                             className="w-full h-full rounded-full object-cover border border-slate-200 shadow-inner block"
//                           />
//                         ) : (
//                           <div className="w-full h-full rounded-full bg-slate-900 text-white border border-slate-200 shadow-inner flex items-center justify-center font-bold text-sm select-none">
//                             {getInitial(chat.receiver?.username)}
//                           </div>
//                         )}
//                         {!isSeen && (
...
//                   {hasAvatar(activeChat.receiver?.avatar) ? (
//                     <img
//                       src={resolveAvatar(activeChat.receiver?.avatar)}
//                       alt=""
//                       className="w-full h-full rounded-full object-cover border border-slate-200 shadow-sm block"
//                     />
//                   ) : (
//                     <div className="w-full h-full rounded-full bg-slate-900 text-white border border-slate-200 shadow-sm flex items-center justify-center font-bold text-sm select-none">
//                       {getInitial(activeChat.receiver?.username)}
//                     </div>
//                   )}
//                 </div>
...
const ProfilePage = () => {
  const navigate = useNavigate();
  const location = useLocation(); // 💎 State receive karne ke liye useLocation lagaya
  const { user: currentUser, updateUser } = useUser();
  const userId = currentUser?.id || currentUser?.userId || currentUser?._id;
  ...
  // 🎯 💎 REDIRECTION INTERCEPTOR (AUTO OPEN CHAT LAYER)
  useEffect(() => {
    if (conversations.length > 0 && !loadingChats) {
      // Pehla check: Agar direct dynamic autoOpenChatId bhej di gayi ho state mein
      if (location.state?.autoOpenChatId) {
        const targetChatId = String(location.state.autoOpenChatId);
        const matchingChat = conversations.find(
          (c) => String(c.id || c._id) === targetChatId
        );

        if (matchingChat) {
          handleOpenChat(matchingChat);
          // State ko reset karte hain taake refresh par baar baar trigger na ho
          navigate(location.pathname, { replace: true, state: {} });
        }
      } 
      // Dusra check: Agar sirf post owner ki receiverId mili ho
      else if (location.state?.receiverId) {
        const targetReceiverId = String(location.state.receiverId);
        const matchingChat = conversations.find(
          (c) => String(c.receiver?.id || c.receiver?._id || c.receiver?.userId) === targetReceiverId
        );
  ...
  }, [location.state, conversations, loadingChats]);
